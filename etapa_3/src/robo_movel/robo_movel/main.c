...
/* Application mode */
static bool g_standby_mode = true;
...
static bool timer_callback(struct repeating_timer *t)
{
    ...
    /* Read left color sensor (I2C0) */
    ColorData data_left = tcs_read(i2c0);
    if (data_left.valid) {
        g_color_left = color_identify(data_left);
    }

    /* Read right color sensor (I2C1) */
    ColorData data_right = tcs_read(i2c1);
    if (data_right.valid) {
